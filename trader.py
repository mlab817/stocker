...
def generate_signals(buy_condition, sell_condition, dataframe, strategy):
    last_signal = None
    indicators = []
    buy = []
    sell = []
    ...
    for i in range(0, len(dataframe)):
        if buy_condition(i, dataframe) and last_signal != 'Buy':
            last_signal = 'Buy'
            indicators.append(last_signal)
            buy.append(dataframe['Close'].iloc[i])
            sell.append(np.nan)
        elif sell_condition(i, dataframe) and last_signal == 'Buy':
            last_signal = 'Sell'
            indicators.append(last_signal)
            buy.append(np.nan)
...
def get_macd(config, company):
    ...
    slow, signal, fast = 26, 9, 12
    macd = MACD(company.prices, slow, fast, signal)
